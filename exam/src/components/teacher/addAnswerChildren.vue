  ...
  <div class="add">
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane name="second">
        <span slot="label"
          ><i class="iconfont iconkaoshixinxi"></i>在线组卷</span
        >
        <div class="box">
          <ul>
            <li>
...
export default {
  ...
  methods: {
    ...
    dealFile: function (data) {
      ...
      this.imFile.value = "";
      this.fullscreenLoading = false;
      console.log(data[0].subject, this.subject);
      if (data.length <= 0 || data[0].subject != this.subject) {
        this.$message({
          message: `需要${this.subject}题库，错误提供${data[0].subject}题`,
          type: "error",
        });
      } else {
        this.excelData = data;
        ...
        while (this.excelData[i] != null) {
          if (this.type == "multi") {
            this.postChange = this.excelData[i];
            this.changeSubmit();
          } else if (this.type == "judge") {
            this.judgeChange = this.excelData[i];
            this.judgeSubmit;
          } else if (this.type == "fill") {
            this.fillChange = this.excelData[i];
            this.fillSubmit;
          }
          i++;
        }
